...
@numba.njit()
def rasterize_lambertian_textureless(coords, view,zbuffer,normal,coords_3d,LIGHTS,intensities,colors):
...
@numba.njit()
def rasterize_lambertian_textured(coords, view,zbuffer,normal,coords_3d,texturecoords,texture,LIGHTS,intensities,colors):
...
@numba.njit()
def rasterize_gouraud_textureless(coords, view,zbuffer,normals,coords_3d,LIGHTS,intensities,colors):
                ...
                for i, LIGHT_POS in enumerate(LIGHTS):
                    ...
                    color = np.array(3 * [min(255, 255*(
                            ...
                            max(n3.dot(point_to_light), 0.0) * gamma) +
                                                  parameters.AMBIENT_INTENSITY)])*intensities[i]*(colors[i])
...
@numba.njit()
def rasterize_gouraud_textured(coords, view,zbuffer,normals,coords_3d,texturecoords,texture,LIGHTS,intensities,colors):
...
def rasterize_phong_textured(coords, view, zbuffer, av_normals, coords_3d,LIGHTS, intensities,colors,texturecoords=None, texture=np.empty((1,1),np.int64),):
...
@numba.njit()
def rasterize_phong_textureless(coords, view, zbuffer, av_normals, coords_3d,LIGHTS,intensities, colors,):
